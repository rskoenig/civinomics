...
class TestAccountController(TestController): 
    ...
    def test_change_hosted_accounts_facilitator(self):
        ...
        logout(self)
        user_admin_page = login_and_view_user_admin_page(self, TestAccountController.adminEmail, TestAccountController.adminPass, TestAccountController.adminController, TestAccountController.adminAction, TestAccountController.first2, TestAccountController.last2)
    ...
    def test_change_hosted_accounts_new_admin(self):
        ...
        # set this user's access level to facilitator
        adminPrivsRes = login_and_set_user_auth_level(self, TestAccountController.adminEmail, TestAccountController.adminPass, TestAccountController.adminController, TestAccountController.adminAction, TestAccountController.first3, TestAccountController.last3, TestAccountController.user3AccessLevel)
        # logout
        logout(self)
        # login as this new user
        login = login_user(self, TestAccountController.user3, TestAccountController.pass3)
        # visit my profile page and obtain code/name
        landing_page = self.app.get(url=url(controller='actionlist', action='index', id='sitemapIssues'))
        profile_page = landing_page.click("Profile")
        ...
        # if it works, the database will reflect this new numHost value. login as admin and take a look
        logout(self)
        user_admin_page = login_and_view_user_admin_page(self, TestAccountController.adminEmail, TestAccountController.adminPass, TestAccountController.adminController, TestAccountController.adminAction, TestAccountController.first3, TestAccountController.last3)
        # since this was a facilitator trying to set hosting accounts, this should not have gone through.
        # Therefore, the previous test string will not be there and instead the one that prints when no 
        # hosted accounts are set will be:

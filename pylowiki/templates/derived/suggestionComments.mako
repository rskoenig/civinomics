...
<% issueID = c.i.id %>
 ...
 %if c.conf['allow.comments'] == 'true':
  ...
  <span class="gray right">Last edited on ${c.lastmoddate} by ${c.lastmoduser} | <a href="#featuredComments">${numComments} comments</a> | <a href="#comments">Leave a comment</a></span>
  </div>
  ...
  <h3>Comments</h3>
   ...
   % if not comment.disabled and not comment.pending:
     ...
     <li>
        <span><img src="/images/avatars/${comment.event.user.pictureHash}.thumbnail" /> <a href = "/account/${comment.event.user.name}">${comment.event.user.name}</a> says ...</span>
        <p>
        <br />

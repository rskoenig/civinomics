...
def getActivityCountByObjectForWorkshop(workshopCode, disabled = '0', deleted = '0'):
    """
        Activity inside a single workshop
        Should be rewritten to return a count if that's all we want, and to do the discussion filtering on the db level
    """
    ...
    for activity in initialActivityList:
        #log.info("activity type: %s"%(activity.objType))
        if activity.objType == 'discussion':
            if activity['discType'] == 'general':
                count['discussion'] += 1
            if 'numComments' in activity:
                count['comment'] += int(activity['numComments'])
        elif activity.objType == 'resource':
            #log.info("resource title: %s"%(activity.title))
            count['resource'] += 1
            if 'numComments' in activity:
                count['comment'] += int(activity['numComments'])
        elif activity.objType == 'idea':
            count['idea'] += 1
            if 'numComments' in activity:
                count['comment'] += int(activity['numComments'])
    #log.info("counts %s, %s, %s, %s"%(count["resource"], count["discussion"], count["idea"], count["comment"]))  
    return count
...
def getIdeasForWorkshop(workshopCode, disabled = '0', deleted = '0'):
    """
        Get ideas for a workshop
    """
    #: get the ideas
    stats = {}
    try:
...
def getPostsForWorkshop(workshopCode, disabled = '0', deleted = '0'):
    """
        Get all posts for a workshop

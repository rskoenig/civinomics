...
def commentLinker(comment):
    ...
    """
    Is this for the alerts that someone has commented on something you posted?
        - todd: what other cases could exist for a comment to create a message for me?
    If so:
    1. comments point back to a discussion object as their parent, and the discussion will point to its container, which is usually what
    you want to link to, because comment permalinks aren't really working.
    2. if the discussion discType == 'general' than it is a discussion topic in a workshop, in which case you want to point to the discussion, or,
    if the discussion discType == 'update' than it is an initiative update, in which case you want to point to the discussion again
